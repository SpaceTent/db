...
func TestDataTypes(t *testing.T) {
	...
	t.Run("Integer Types", func(t *testing.T) {
		...
		assert.NoError(t, err)
		assert.Equal(t, int(2147483647), resp.IntVal)
		...
		_, rowsAffected, err = DB.Execute(`
            INSERT INTO Users(id,intval, int8val,int16val,int32val,int64val,uintval,uint8val,uint16val,uint32val,uint64val) 
            VALUES (?,?,?,?,?,?,?,?,?,?,?)`,
			2, -2147483648, -128, -32768, -2147483648, -9223372036854775808,
			0, 0, 0, 0, 0)
		assert.NoError(t, err)
		assert.Greater(t, rowsAffected, int64(0))
		...
		assert.NoError(t, err)
		assert.Equal(t, int(-2147483648), resp.IntVal)
		assert.Equal(t, int8(-128), resp.Int8Val)
		assert.Equal(t, int16(-32768), resp.Int16Val)
		assert.Equal(t, int32(-2147483648), resp.Int32Val)
		assert.Equal(t, int64(-9223372036854775808), resp.Int64Val)
		assert.Equal(t, uint(0), resp.UintVal)
		assert.Equal(t, uint8(0), resp.Uint8Val)
		assert.Equal(t, uint16(0), resp.Uint16Val)
		assert.Equal(t, uint32(0), resp.Uint32Val)
		assert.Equal(t, uint64(0), resp.Uint64Val)
		...
		resp, err = QuerySingleStruct[IntTypes]("SELECT * FROM Users WHERE id=?", 3)
		assert.NoError(t, err)
		assert.Equal(t, 3, resp.Id)
		assert.Equal(t, int(0), resp.IntVal)
		assert.Equal(t, int8(0), resp.Int8Val)
		assert.Equal(t, int16(0), resp.Int16Val)
		assert.Equal(t, int32(0), resp.Int32Val)
		assert.Equal(t, int64(0), resp.Int64Val)
		assert.Equal(t, uint(0), resp.UintVal)
		assert.Equal(t, uint8(0), resp.Uint8Val)
		assert.Equal(t, uint16(0), resp.Uint16Val)
		assert.Equal(t, uint32(0), resp.Uint32Val)
	...
	t.Run("Float Types", func(t *testing.T) {
		...
		assert.Equal(t, float32(0), resp.Float32Val)
		assert.Equal(t, float64(0), resp.Float64Val)
		...
		resp, err = QuerySingleStruct[FloatTypes]("SELECT * FROM Users WHERE id=?", 3)
		assert.NoError(t, err)
		assert.Equal(t, 3, resp.Id)
		assert.Equal(t, float32(0), resp.Float32Val)
		assert.Equal(t, float64(0), resp.Float64Val)
	...
	t.Run("Boolean Type", func(t *testing.T) {
		...
		assert.NoError(t, err)
		assert.Equal(t, 2, resp.Id)
		assert.Equal(t, false, resp.BoolVal)
		...
		resp, err = QuerySingleStruct[BoolType]("SELECT * FROM Users WHERE id=?", 3)
		assert.NoError(t, err)
		assert.Equal(t, 3, resp.Id)
		assert.Equal(t, false, resp.BoolVal)
	...
	t.Run("String Type", func(t *testing.T) {
		...
		_, rowsAffected, err = DB.Execute("INSERT INTO Users(id,stringval) VALUES (?,?)", 2, "")
		assert.NoError(t, err)
		assert.Greater(t, rowsAffected, int64(0))
		resp, err = QuerySingleStruct[IntegrationGenericStruct]("SELECT * FROM Users where id=?", 2)
		assert.NoError(t, err)
		assert.Equal(t, 2, resp.Id)
		assert.Equal(t, "", resp.StringVal)

		_, rowsAffected, err = DB.Execute("INSERT INTO Users(id,stringval) VALUES (?,?)", 3, nil)
		assert.NoError(t, err)
		assert.Greater(t, rowsAffected, int64(0))
		resp, err = QuerySingleStruct[IntegrationGenericStruct]("SELECT * FROM Users where id=?", 3)
		assert.NoError(t, err)
		assert.Equal(t, 3, resp.Id)
		assert.Equal(t, "", resp.StringVal)
	...
	t.Run("Time Type", func(t *testing.T) {
		...
		assert.NoError(t, err)
		assert.Equal(t, time.Time{}.UTC(), resp.TimeVal.UTC())

		// cannot test nil value because time.Now() happens
...
func TestPtrDataTypes(t *testing.T) {
	fname := setUpSaveIntegrationTestConnection(t)
	...
	t.Run("Integer Types", func(t *testing.T) {
		...
		assert.NoError(t, err)
		assert.Equal(t, 2, *resp.Id)
		assert.Equal(t, (*int)(nil), resp.IntVal)
